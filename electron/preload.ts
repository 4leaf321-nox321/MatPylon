...
const api: MatPylonApi = {
  ...
  registerConnector: (url, name, ws, tls) => ipcRenderer.invoke(CHANNELS.registerConnector, url, name, ws, tls),
  listWorkspaces: (url, tls) => ipcRenderer.invoke(CHANNELS.listWorkspaces, url, tls),
  resolveHints: (hints) => ipcRenderer.invoke(CHANNELS.resolveHints, hints),
  getAutoRegister: () => ipcRenderer.invoke(CHANNELS.getAutoRegister),
  setAutoRegister: (enabled) => ipcRenderer.invoke(CHANNELS.setAutoRegister, enabled),
  reference: () => ipcRenderer.invoke(CHANNELS.reference),
  ...
  previewPaths: (source, limit) => ipcRenderer.invoke(CHANNELS.previewPaths, source, limit),
